...
pub fn housesEx2(
    ...
    const ret_flag = sweph.swe_houses_ex2(
        tjd_ut,
        iflag,
        geolat,
        geolon,
...
test "housesEx2" {
    var eql_0_aries_houses = try housesEx2(testing.allocator, 2440587.5, sweph.SEFLG_SPEED, 0, 0, 'N', undefined);
    defer eql_0_aries_houses.deinit(testing.allocator);
    try testing.expectEqual(12, eql_0_aries_houses.cusps.len);
    ...
    var gauquelin_sectors = try housesEx2(testing.allocator, 2440587.5, sweph.SEFLG_SPEED, 0, 0, 'G', undefined);
    defer gauquelin_sectors.deinit(testing.allocator);
    try testing.expectEqual(36, gauquelin_sectors.cusps.len);
    ...
    const svalbard_lat = 78.22;
    const svalbard_lon = 15.65;
    var porphyry_houses = try housesEx2(testing.allocator, 2440587.5, sweph.SEFLG_SPEED, svalbard_lat, svalbard_lon, 'P', &diags);
    defer porphyry_houses.deinit(testing.allocator);
    defer diags.deinit();
    try testing.expect(diags.err != null);
    if (diags.err) |err| {
...
pub fn housesArmc(
    ...
    const ret_flag = sweph.swe_houses_armc(
        armc,
        geolat,
        eps,
        hsys,
...
pub const SolarEclipseAttributes = struct {
...
pub fn Eclipse(comptime T: type) type {
    return struct {
        type: i32,
        position: struct {
            latitude: f64,
...
const LunarOccultationAttributes = struct {
    /// Fraction of solar diameter covered by moon (magnitude)
    magnitude: f64,
    ...
    /// Azimuth of object at time of eclipse
    obj_azimuth: f64,

    /// True altitude of object above horizon
    obj_true_altitude: f64,

    /// Apparent altitude of object above horizon
    obj_apparent_altitude: f64,

    /// Angular distance of moon from object in degrees
    moon_angular_distance: f64,
};

pub fn lunOccultWhere(
    tjd: f64,
    ipl: i32,
    starname: ?[]const u8,
    ifl: i32,
    ...
    return .{
        ...
        .attributes = .{
            ...
            .core_shadow_diameter_km = attr[3],
            .obj_azimuth = attr[4],
            .obj_true_altitude = attr[5],
            .obj_apparent_altitude = attr[6],
            .moon_angular_distance = attr[7],
        },
    };
}
